...
import { WORKCOSMO_MODULES, buildModuleUrl, isModuleEnabled, normalizeClientId } from "./modules.js";
...
function renderModules(company) {
    const cid = normalizeClientId(company?.companyId || company?.clientId || company?.subdomain || company?.id || "");
    els.modulesGrid.innerHTML = WORKCOSMO_MODULES.map((mod) => {
        ...
        const stateLabel = live ? "Launch" : enabled ? "Coming soon" : "Not enabled";
...
function renderDashboard(user, profile, company) {
    const cid = normalizeClientId(company?.companyId || profile?.companyId || company?.id || "");
    els.companyName.textContent = company?.companyName || company?.name || "Workcosmo Workspace";
...
async function handleSignedIn(user) {
    ...
    const profileCompanyId = normalizeClientId(profile.companyId || profile.clientId || profile.subdomain);
    const requested = normalizeClientId(sessionStorage.getItem("tenant_client_id") || els.clientId.value || profileCompanyId);
